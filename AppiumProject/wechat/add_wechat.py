...
class AddWechat(object):
    ...
    def send_verfiy_message(self):
        try:
            ...
            if Locators.ADD_NEWS[1] in self.driver.page_source:
                ...
                #微信会弹出操作频繁提示，点击添加到通讯录之后，判断界面是否存在‘添加到通讯录文案’，如果存在则继续下一个
                ...
                try:
                    ...
                    if Locators.ADD_NEWS[1] in self.driver.page_source:
                        ...
                        self.write_count(AddWechat, self.number)
                        self.select_member(AddWechat)
                    elif Locators.VARIFY_APPLY[1] in self.driver.page_source:
                        self.driver.find_element(Locators.VARIFY_SEND[0], Locators.VARIFY_SEND[1]).click()
                        WebDriverWait(self.driver, self.wait_time).until(
                            lambda driver: driver.find_element(Locators.ADD_CONTACT[0], Locators.ADD_CONTACT[1]))
                        os.popen(self.back_cmd)
    ...
    def select_member(self):
        ...
        if self.udid == '5761c059':
            ...
            if self.qun_num - self.read_count(self) <= 21:
                chu_shu = self.temp_num / 4
                yu_shu = self.temp_num % 4

                click_cmd = 'adb -s %s shell input tap %d %d' % (
                    self.udid, InitPara.no_swpie_x + yu_shu * InitPara.no_swpie_x_dur,
                    InitPara.no_swpie_y  + chu_shu * InitPara.no_swpie_y_dur)
                os.popen(click_cmd)
                time.sleep(1.0)
                self.temp_num += 1
                ...
                for i in range(0, num1):
                    ...
                    self.udid, InitPara.vivo_user_swipe_x, InitPara.vivo_user_swipe_y1,
            ...
            for i in range(0, num1):
                swipe_cmd = 'adb -s %s shell input swipe %d %d %d %d %d' % (
                    self.udid, InitPara.xiaomi_user_swipe_x, InitPara.xiaomi_user_swipe_y1,
                    InitPara.xiaomi_user_swipe_x, InitPara.xiaomi_user_swipe_y2, InitPara.xiaomi_swipe_time)
                os.popen(swipe_cmd)
                time.sleep(1.0)

            if self.qun_num - self.read_count(self) <= 21:
                chu_shu = self.temp_num / 5
                yu_shu = self.temp_num % 5
                ...
                if num2 == 0:
                    ...
                    click_cmd = 'adb -s %s shell input tap %d %d' % (
                        self.udid, InitPara.xiaomi_user_x + InitPara.xiaomi_user_x_dur * num2,
                        InitPara.xiaomi_user_y)
                    os.popen(click_cmd)
                    time.sleep(1.0)
    ...
    def read_count(self):
        file_name = 'E:\\pyproject\\AppiumProject\\wechat\\send_count'
